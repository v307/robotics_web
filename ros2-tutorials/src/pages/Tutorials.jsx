...
export default function Tutorials() {
  ...
  const [tutorials, setTutorials] = useState([]); // <-- fetched data
  ...
  // Fetch tutorials from Django API
  useEffect(() => {
    const fetchTutorials = async () => {
      try {
        const response = await fetch("http://127.0.0.1:8000/api/tutorials/"); 
        if (!response.ok) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-cyan-50">
      ...
      <div
        className="fixed bottom-1/3 right-1/4 w-96 h-96 bg-cyan-200/20 rounded-full blur-3xl animate-pulse-slow"
        ...
        <div className="max-w-6xl mx-auto text-center">
          ...
          >
            <div className="inline-flex items-center space-x-2 bg-white/80 backdrop-blur-sm px-4 py-2 rounded-full shadow-lg mb-6 hover:bg-white hover:shadow-xl transition-all duration-300 cursor-pointer group">
              ...
              <span className="text-slate-600 font-medium group-hover:text-blue-600 transition-colors duration-300">
                Learn & Master
              </span>
            </div>
            ...
            <p className="text-xl text-slate-600 max-w-3xl mx-auto mb-8 hover:text-slate-700 transition-colors duration-300 cursor-default">
              Comprehensive step-by-step tutorials to master Robot Operating
              System 2. From beginner basics to advanced robotics concepts.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="px-6 pb-20 relative z-10">
        <div className="max-w-7xl mx-auto">
          {loading && (
            <div className="text-center py-16 text-slate-600 text-xl">
              Loading tutorials...
            </div>
          )}
          {error && (
          ...
          {!loading && !error && (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {filteredTutorials.map((tutorial, index) => (
                ...
                >
                  <div
                    className={`absolute inset-0 bg-gradient-to-br from-blue-100/30 to-cyan-100/30 rounded-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 ${
                  ...
                  <div className="p-6 pb-4 relative z-10">
                    <div className="flex items-center justify-between mb-4">
                      <span
                        className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold text-white bg-gradient-to-r ${getDifficultyColor(
                          tutorial.difficulty
